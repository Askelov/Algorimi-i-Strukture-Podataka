...
template<typename TipK, typename TipV>
void HashMapa<TipK,TipV>:: povecaj_kapacitet(){ //povecamo ako je mal kapacitet pri alokaciji
    kapacitet*=2; //duplo
...
template<typename TipK, typename TipV>
HashMapa<TipK,TipV>::~HashMapa(){ //desturktor 
    for(int i=0;i<kapacitet;i++){
        if(parovi[i]!=nullptr)
            delete parovi[i];}
        delete[] parovi;
...
TipV HashMapa<TipK,TipV>:: operator [](const TipK& kljuc) const {
    if(!h)
        throw"greska";
        unsigned int hash=h(kljuc,kapacitet); //hesh
        int indeks;
        for(int i=0;i<kapacitet;i++){
...
template <typename TipK, typename TipV>
void HashMapa<TipK,TipV>::obrisi(){ //ko dekst
    for(int i=0;i<kapacitet;i++){
        if(parovi[i]!=nullptr){
            delete parovi[i];
            parovi[i]=nullptr;
