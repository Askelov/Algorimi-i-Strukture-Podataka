...
class NizMapa : public Mapa<TipK, TipV>{
    ...
    Par** parovi;
    int kapacitet,broj_el;
    void povecaj();
public:
    NizMapa() : Mapa<TipK, TipV>(), parovi(new Par*[VELICINA]), kapacitet(VELICINA), broj_el(0){}
    ~NizMapa();
...
template <typename TipK, typename TipV>
void NizMapa<TipK,TipV>::povecaj(){
    kapacitet=kapacitet*2;
    Par** novi= new Par*[kapacitet];
    for(int i=0;i<broj_el;i++){
        TipK kljuc = parovi[i]->key;
        TipV vrijednost=parovi[i]->value;
        novi[i]= new Par(kljuc,vrijednost);
        delete parovi[i];
    }
    delete[] parovi;
    parovi=novi;
...
NizMapa<TipK,TipV>::NizMapa(const NizMapa<TipK,TipV>&nmapa){
    ...
    for(int i=0;i<broj_el;i++){
        TipK kljuc=nmapa.parovi[i]->key;
        TipV vrijednost=nmapa.parovi[i]->value;
        parovi[i]= new Par(kljuc,vrijednost);
    }
}
